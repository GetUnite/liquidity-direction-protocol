...
async function main() {
    ...
    // console.log(await pseudoMultiSig.getOwners());
    ...
    // await alluoVoteExecutorUtils.setCrossChainInformation(alluoVoteExecutor.address, alluoVoteExecutor.address, alluoVoteExecutor.address, 10, 137, 137, 2, 0)

    let func5 = alluoVoteExecutor.interface.encodeFunctionData("setAcrossInformation", [spokePool, ethers.utils.parseUnits("4", 17)])
    let func6 = alluoVoteExecutor.interface.encodeFunctionData("setFeeInformation", [usdc.address, 137, ethers.utils.parseUnits("5", 6)])
