...
async function main() {
    ...
    let encodedMessage2 = await alluoVoteExecutorUtils.encodeLiquidityCommand("BeefyMooStargateUsdtPolygon", 10000, 1);
    ...
    let encodedMessage5 = await alluoVoteExecutorUtils.encodeLiquidityCommand("BeefyStETHOptimism", 0, 0);
    let encodedMessage6 = await alluoVoteExecutorUtils.encodeLiquidityCommand("BeefyFrxETHOptimism", 0, 0);
    let encodedMessage7 = await alluoVoteExecutorUtils.encodeLiquidityCommand("BeefyaTriCrypto3EthPolygon", 10000, 1);
    let encodeAllMessages = await alluoVoteExecutorUtils.encodeAllMessages([encodedMessage1[0], encodedMessage2[0], encodedMessage3[0], encodedMessage4[0], encodedMessage5[0], encodedMessage6[0], encodedMessage7[0]], [encodedMessage1[1], encodedMessage2[1], encodedMessage3[1], encodedMessage4[1], encodedMessage5[1], encodedMessage6[1], encodedMessage7[1]]);
    ...
    //
    //
    let tx = await alluoVoteExecutor.approveSubmittedData(15, [signature])
    await tx.wait()
    await alluoVoteExecutor.executeSpecificData(15)
}
