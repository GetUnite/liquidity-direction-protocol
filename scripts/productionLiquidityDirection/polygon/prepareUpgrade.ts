...
async function main() {
    ...
    let newStrategyHandler = await ethers.getContractFactory("AlluoStrategyHandler");
    let newContract = await upgrades.prepareUpgrade(alluoStrategyHandler.address, newStrategyHandler);
