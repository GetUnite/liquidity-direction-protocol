...
async function main() {
    ...
    // })
    const gnosis = await ethers.getSigner("0xc7061dD515B602F86733Fa0a0dBb6d6E6B34aED4");
    ...
    console.log("Contracts above deployed at addresses:")
    console.log("wethStrategy", wethStrategy.address);
    console.log("fraxStrategy", fraxStrategy.address);
    console.log("wbtcStrategy", wbtcStrategy.address);
    console.log("maiStrategy", maiStrategy.address);
    console.log("frxEthStrategy", frxEthStrategy.address);
    console.log("mooVelodromeMAIUSDCStrategy", mooVelodromeMAIUSDCStrategy.address);
    console.log("mooVelodromeDOLAFRAXStrategy", mooVelodromeDOLAFRAXStrategy.address);
    console.log("mooVelodromeDOLAMAIStrategy", mooVelodromeDOLAMAIStrategy.address);


    console.log("Now print the raw transactions that we need to execute via gnosis")
    console.log("Weth strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [wethStrategy.address, weth.address]));
    console.log("Frax strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [fraxStrategy.address, frax.address]));
    console.log("Wbtc strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [wbtcStrategy.address, wbtc.address]));
    console.log("Mai strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [maiStrategy.address, mai.address]));
    console.log("FrxEth strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [frxEthStrategy.address, frxEth.address]));
    console.log("MooVelodromeMAIUSDC strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [mooVelodromeMAIUSDCStrategy.address, mooVelodromeMAIUSDC.address]));
    console.log("MooVelodromeDOLAFRAX strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [mooVelodromeDOLAFRAXStrategy.address, mooVelodromeDOLAFRAX.address]));
    console.log("MooVelodromeDOLAMAI strategy connect:", priceRouter.interface.encodeFunctionData("setCryptoStrategy", [mooVelodromeDOLAMAIStrategy.address, mooVelodromeDOLAMAI.address]));
    ...
    // const cryptos = [frxEth, mai, frax, weth, wbtc, mooVelodromeMAIUSDC, mooVelodromeDOLAFRAX, mooVelodromeDOLAMAI];
...
// npx hardhat run scripts/deploy/priceFeeds/workbench.ts --network optimism