...
import {IBalancerStructs, IBalancer} from "./../interfaces/IBalancer.sol";
import {ICvxDistributor} from "./../interfaces/ICvxDistributor.sol";
import {IAlluoLockedV3} from "./../interfaces/IAlluoLockedV3.sol";
    ...
    IBalancerStructs
{
    ...
    uint256 public constant DISTRIBUTION_TIME = 86400;
    ...
    ICvxDistributor public cvxDistributor;
    ...
    IERC20Upgradeable public constant ALLUO_TOKEN =
    ...
    IERC20Upgradeable public constant ALLUO_BALANCER_LP =
    ...
    function lock(uint256 _amount) public {
        ...
        ALLUO_TOKEN.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function migrateWithdrawOrClaimValues(
        ...
        IAlluoLockedV3 oldLocker = IAlluoLockedV3(
            0xF295EE9F1FA3Df84493Ae21e08eC2e1Ca9DebbAf
        );
    ...
    function withdraw() public whenNotPaused {
        ...
        if (_oldWithdraw != 0) {
            ALLUO_TOKEN.safeTransfer(msg.sender, _oldWithdraw);
        ...
        ALLUO_TOKEN.safeTransfer(msg.sender, amount);
    ...
    function claim() public {
        ...
        if (oldReward > 0) {
            ALLUO_TOKEN.safeTransfer(msg.sender, oldReward);
        ...
        if (reward > 0) {
            ...
            ALLUO_TOKEN.safeTransfer(msg.sender, reward);
    ...
    function setCvxDistributor(
        address cvxDistributorAddress
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        cvxDistributor = ICvxDistributor(cvxDistributorAddress);
    }
