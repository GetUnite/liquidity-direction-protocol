...
contract BtcCurveAdapter is AccessControl {
    ...
    constructor(
        ...
        IERC20(RENBTC).safeApprove(CURVE_POOL, type(uint256).max);
        ...
        if (_token == primaryToken) {
            ...
            if (_leaveInPool != 0) {
                ...
                amounts[primaryTokenIndex] =
                    _leaveInPool /
                    10 ** (18 - IERC20Metadata(primaryToken).decimals());
                ICurvePoolBTC(CURVE_POOL).add_liquidity(amounts, 0, true);
            }
        } else {
            uint256[2] memory amounts;
    ...
    function withdraw(
        ...
        uint256[2] memory amounts;
        address liquidToken = ICurvePoolBTC(CURVE_POOL).underlying_coins(
            liquidTokenIndex
        );
        ...
        if (_token == liquidToken) {
            ...
            uint256 minAmountOut = _amount /
                10 ** (18 - IERC20Metadata(_token).decimals());
            uint256 toUser = ICurvePoolBTC(CURVE_POOL)
    ...
    function getAdapterAmount() external view returns (uint256) {
        ...
        if (curveLpAmount != 0) {
            ...
            uint256 amount = ICurvePoolBTC(CURVE_POOL).calc_withdraw_one_coin(
                curveLpAmount,
    ...
    {
        return (
            ICurvePoolBTC(CURVE_POOL).underlying_coins(liquidTokenIndex),
            ICurvePoolBTC(CURVE_POOL).underlying_coins(primaryTokenIndex)
        );
    }
