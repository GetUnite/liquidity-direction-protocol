...
contract Superfluid is UUPSProxiable, ISuperfluid, BaseRelayRecipient {
     ...
     */
    // solhint-disable-next-line var-name-mixedcase
    uint public immutable MAX_APP_LEVEL = 1;

    // solhint-disable-next-line var-name-mixedcase
    uint64 public immutable CALLBACK_GAS_LIMIT = 3000000;
    ...
    function updateCode(address newAddress) external override onlyGovernance {
        ...
        require(
            !Superfluid(newAddress).NON_UPGRADABLE_DEPLOYMENT(),
    ...
    function registerAgreementClass(
        ...
        require(
            _agreementClassIndices[agreementType] == 0,
            "SF: agreement class already registered"
        );
        require(
            _agreementClasses.length < 256,
    ...
    function _registerApp(
        ...
        require(
            _appManifests[ISuperApp(app)].configWord == 0,
    ...
    function getAppLevel(
        ...
        return
            SuperAppDefinitions.getAppLevel(_appManifests[appAddr].configWord);
    }
    ...
    function isAppJailed(ISuperApp app) public view override returns (bool) {
    ...
    function allowCompositeApp(ISuperApp targetApp) external override {
        ...
        require(
            getAppLevel(sourceApp) > getAppLevel(targetApp),
        ...
        returns (bytes memory appCtx)
    {
        ...
        if (isApp(ISuperApp(context.msgSender))) {
            require(
                _compositeApps[ISuperApp(context.msgSender)][app],
    ...
    function _updateContext(
        ...
        require(
            context.appLevel <= MAX_APP_LEVEL,
        ...
        uint256 allowanceIO = context.appAllowanceGranted.toUint128() |
    ...
    function _decodeCtx(
        ...
        (ctx1, ctx2) = abi.decode(ctx, (bytes, bytes));
        {
            ...
            (context.appLevel, context.callType) = ContextDefinitions
    ...
    modifier isAgreement(ISuperAgreement agreementClass) {
        require(
            isAgreementClassListed(agreementClass),
    ...
    modifier onlyAgreement() {
        require(
            isAgreementClassListed(ISuperAgreement(msg.sender)),
