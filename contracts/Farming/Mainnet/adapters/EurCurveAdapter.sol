...
contract EurCurveAdapterMainnet is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function adapterApproveAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
        IERC20(AGEUR).safeApprove(CURVE_POOL, type(uint256).max);
    ...
    function withdraw(
        ...
        uint256[3] memory amounts;
        address liquidToken = ICurvePoolEUR(CURVE_POOL).coins(liquidTokenIndex);
    ...
    function getAdapterAmount() external view returns (uint256) {
        ...
        if (curveLpAmount != 0) {
            address liquidToken = ICurvePoolEUR(CURVE_POOL).coins(
                liquidTokenIndex
            );
            uint256 amount = ICurvePoolEUR(CURVE_POOL).calc_withdraw_one_coin(
                curveLpAmount,
