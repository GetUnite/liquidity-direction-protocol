...
contract UsdCurveAdapterMainnet is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function deposit(
        ...
        if (_token == primaryToken) {
            ...
            uint256 lpAmountBefore = IERC20(CURVE_LP).balanceOf(address(this));
            ICurvePoolUSD(CURVE_POOL).add_liquidity(amounts, 0);
            uint256 delta = IERC20(CURVE_LP).balanceOf(address(this)) -
                lpAmountBefore;
    ...
    function withdraw(
        ...
        uint256[3] memory amounts;
        address liquidToken = ICurvePoolUSD(CURVE_POOL).coins(liquidTokenIndex);
        ...
        if (_token == liquidToken) {
            ...
            ICurvePoolUSD(CURVE_POOL).remove_liquidity_one_coin(
                toBurn,
    ...
    function getAdapterAmount() external view returns (uint256) {
        ...
        if (curveLpAmount != 0) {
            address liquidToken = ICurvePoolUSD(CURVE_POOL).coins(
                liquidTokenIndex
            );
    ...
    {
        return (
            ICurvePoolUSD(CURVE_POOL).coins(liquidTokenIndex),
            ICurvePoolUSD(CURVE_POOL).coins(primaryTokenIndex)
        );
    }
