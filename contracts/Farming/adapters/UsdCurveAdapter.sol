...
contract UsdCurveAdapter is AccessControl {
    ...
    address public constant curveLp =
    ...
    function deposit(
        ...
        address primaryToken = ICurvePoolUSD(curvePool).underlying_coins(
            primaryTokenIndex
        );
        if (_token == primaryToken) {
            ...
            uint256 lpAmount = ICurvePoolUSD(curvePool).add_liquidity(
                amounts,
                0,
                true
            );
    ...
    function withdraw(
        ...
        uint256[3] memory amounts;
        address liquidToken = ICurvePoolUSD(curvePool).underlying_coins(
            liquidTokenIndex
        );
        ...
        if (_token == liquidToken) {
            ...
            uint256 toBurn = ICurvePoolUSD(curvePool).calc_token_amount(
                amounts,
                false
            );
    ...
    function getAdapterAmount() external view returns (uint256) {
        ...
        if (curveLpAmount != 0) {
            ...
            uint256 amount = ICurvePoolUSD(curvePool).calc_withdraw_one_coin(
                curveLpAmount,
    ...
    function changeLiquidTokenIndex(uint128 _newLiquidTokenIndex)
        external
