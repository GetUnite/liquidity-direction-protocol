...
contract SuperfluidResolver is AccessControl {
    ...
    function removeFromChecker(address _sender, address _receiver) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _isUserCloseToLiquidation(address _sender, address _token) internal view returns (bool dangerZone) {
        (, int96 flowRate,,) = IConstantFlowAgreementV1(CFAContract).getAccountFlowInfo(ISuperfluidToken(_token), _sender);
        (int256 realTimeBalance,,,) = ISuperfluidToken(_token).realtimeBalanceOfNow(_sender);
    ...
    function liquidateSender(address _sender, address[] memory _receivers, address _token) external onlyRole(GELATO) {
        for (uint256 i; i < _receivers.length; i++) {
            IIbAlluo(_token).pauseFlow(_sender, _receivers[i]);
        }
    }
}
