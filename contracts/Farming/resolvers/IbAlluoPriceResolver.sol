...
contract IbAlluoUSDPriceResolver is AccessControl {
    ...
    function emitter() external {
        ...
        emit IbAlluoUSDValue(
            alluoBank,
            IIbAlluo(ibAlluoAddress).getBalance(alluoBank),
            IIbAlluo(ibAlluoAddress).balanceOf(alluoBank),
            IIbAlluo(ibAlluoAddress).growingRatio()
        );
    }
}
