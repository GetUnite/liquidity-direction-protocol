...
import "../../../interfaces/curve/optimism/ICurvePoolBTC.sol";
    ...
    UUPSUpgradeable
{
    ...
    function adapterApproveAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
        IERC20(SBTC).safeApprove(CURVE_POOL, type(uint256).max);
    ...
    function deposit(
        ...
        address primaryToken = ICurvePoolBTC(CURVE_POOL).coins(
            primaryTokenIndex
        );
    ...
    function getAdapterAmount() external view returns (uint256) {
        ...
        if (curveLpAmount != 0) {
            ...
            uint256 amount = (
                ICurvePoolBTC(CURVE_POOL).calc_withdraw_one_coin(
                    curveLpAmount,
    ...
    function getCoreTokens() external view returns (address primaryToken) {
        return (ICurvePoolBTC(CURVE_POOL).coins(primaryTokenIndex));
    }
