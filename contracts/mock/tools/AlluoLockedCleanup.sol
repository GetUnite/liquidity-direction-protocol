...
contract AlluoLockedCleanup is
    Initializable,
    UUPSUpgradeable,
    ...
    IBalancerStructs
{
    ...
    function clean(address[] calldata users) external onlyRole(UPGRADER_ROLE) {
        for (uint256 i = 0; i < users.length; i++) {
            emit Transfer(users[i], address(0), _lockers[users[i]].amount);
        }
    }
